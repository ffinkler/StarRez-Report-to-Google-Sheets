function populateSheetWithCSV(sheet, csvUrl, requestBody) {
    ...
    // Parse the response as a CSV and replace escaped line breaks with Google compatible version
    var csvContent = Utilities.parseCsv(resp.getContentText().replace(/\\r\\n/g,"\r"));
